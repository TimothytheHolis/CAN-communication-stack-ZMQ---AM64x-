 ...
 *
 * This file is based on the CANopenNode example template canopennode/example/main_blank.c (originally main_generic.c)
...
CANopenNode* canopenNode = NULL; /* It will be set in canopen_app_init and used in other functions */
...
int canopen_app_init(CANopenNode* _canopenNode)
{
    canopenNode = _canopenNode;
...
int canopen_app_resetCommunication()
{
    ...
    /* initialize CANopen */
    err = CO_CANinit(CO, canopenNode, 0);
    if (err != CO_ERROR_NO)
    {
...
#if (((CO_CONFIG_LSS) & (CO_CONFIG_LSS_SLAVE | CO_CONFIG_LSS_MASTER)) != 0)
